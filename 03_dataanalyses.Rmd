...
surv_data = surv_data %>%
  filter(sex != 3) %>% #Exlude sex == 3
  mutate(single_end_d = ifelse(single_end_d == 0, 1, single_end_d)) # two people started a new relationship at the same day they ended their old one, survival analyses can not believe that --> so we set it at 1 day
```
...
##################
surv_data = surv_data %>%
  mutate(single_end_d = ifelse(single_end_d == 0, 1, single_end_d))
...
mod.3 <- coxph(Surv(single_start_d, single_end_d, event_start) ~ 
                            NARQ_ADM_z + cluster(session), data = surv_data)
