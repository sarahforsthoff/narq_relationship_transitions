...
# Previous test run of coxph-function
## Example of cox function with simulated data
```{r}
library(survival)
library(coxme)
```

#Survival analyses with cox regression

#Preperations before analyses
  - event-variable: binary variable indicating if relationship (-> event) happened or not
                    -> so far the data includes the variable "kind", which represents the relationship typ
  - start-variables: it appears as the coxph-function needs numeric time information (e.g. number of days) and not  
                      dates
...
x <- coxph(Surv(single_start_d, relationship_start_d, event) ~ sex, data = surv_data)
summary(x)
plot(survfit(x), mark.time = T)
