...
## TODO before
- data is sound?
- How to interpret results? (continous vs. groups)
...
#Preperations before analyses
  - event-variable: binary variable indicating if relationship (-> event) happened or not
                    -> so far the data includes the variable "kind", which represents the relationship type
  - start-variables: it appears as the coxph-function needs numeric time information (e.g. number of days) and not                      dates
...
###continous
```{r}
start_narq_con.1 <- coxph(Surv(single_start_d, relationship_start_d, event) ~ 
                            NARQ_scale + cluster(session), data = surv_data)
summary(start_narq_con.1)
plot(survfit(start_narq_con.1))


start_narq_con.2 <- coxph(Surv(single_start_d, relationship_start_d, event) ~ 
                            NARQ_ADM + NARQ_RIV + cluster(session), data = surv_data)
summary(start_narq_con.2)
plot(survfit(start_narq_con.2))
testPH_start_narq_con.2 <- ggcoxzph(start_narq_con.2)
ggcoxzph(testPH_start_narq_con.2)

start_narq_con.3 <- coxph(Surv(single_start_d, relationship_start_d, event) ~ 
