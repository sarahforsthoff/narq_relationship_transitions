...
- survival Grafiken nur für Admiration / Rivalry
...
# Rivalry
surv_data = surv_data %>%
  mutate(NARQ_RIV_extreme = ifelse(NARQ_ADM <= RIV_qu_25, "low",
                                     ifelse(NARQ_ADM >= RIV_qu_75, "high", NA)))
...
Sim_rh_start_narq_con.2 <- coxsimLinear(rh_start_narq_con.2, b = "NARQ_ADM_z",
                                        Xj = seq(-3, 3, by = 0.5), nsim = 100)
simGG(Sim_rh_start_narq_con.2, xlab = "Units of admiration from mean in SD", ylab = "Relative Hazards Comparing to Mean")

Sim_rh_start_narq_con.2 <- coxsimLinear(rh_start_narq_con.2, b = "NARQ_RIV_z",
                                        Xj = seq(-3, 3, by = 0.5), nsim = 100)
simGG(Sim_rh_start_narq_con.2, xlab = "Units of rivalry from mean in SD", ylab = "Relative Hazards Comparing to Mean")
```
...
### Plot Relative Hazards for ADM und RIV
```{r}
rh_end_narq_con.2 <- coxph(Surv(rel_start_d, rel_end_d, event_end) ~ 
                            NARQ_ADM_z + NARQ_RIV_z + cluster(session), data = surv_data)

Sim_rh_end_narq_con.2 <- coxsimLinear(rh_end_narq_con.2, b = "NARQ_ADM_z",
                                        Xj = seq(-3, 3, by = 0.5), nsim = 100)
simGG(Sim_rh_end_narq_con.2, xlab = "Units of admiration from mean in SD", ylab = "Relative Hazards Comparing to Mean")

Sim_rh_end_narq_con.2 <- coxsimLinear(rh_end_narq_con.2, b = "NARQ_RIV_z",
                                        Xj = seq(-3, 3, by = 0.5), nsim = 100)
simGG(Sim_rh_end_narq_con.2, xlab = "Units of rivalry from mean in SD", ylab = "Relative Hazards Comparing to Mean")
```
...
plot_model1 = survfit(Surv(rel_start_d, rel_end_d, event_end) ~ 
                            NARQ_ADM_group + NARQ_RIV_group + sex + age + cluster(session),
                          data = surv_data)
fits <- list(NARQ_ADM_group = plot_model1)
ggsurvplot(fits, data = surv_data, conf.int = T)
```
